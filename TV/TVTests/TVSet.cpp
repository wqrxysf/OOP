...
{
    if (!m_isOn || channel < FIRST_CHANNEL || channel > LAST_CHANNEL) return false;
...
bool TVSet::SetChannelName(int channel, const std::string& name) {
    ...
    // избавляемся от старой ассоциации канала
...
{
    if (!m_isOn || channel < FIRST_CHANNEL || channel > LAST_CHANNEL) return std::nullopt;
